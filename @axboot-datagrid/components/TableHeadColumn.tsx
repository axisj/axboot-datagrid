...
const ColumnSorter = styled.span<SorterProps>`
  ...
  &:before {
    ...
    border-top: 4.5px solid var(--axdg-border-color-base);

    ${({ sort }) => {
      if (sort === 'desc') {
        return css`
          border-top: 4.5px solid var(--axdg-primary-color);
        `;
      }
    }}
  }
  ...
  &:after {
    ...
    border-left: 3.6px solid #0000;
    border-right: 3.6px solid #0000;
    border-bottom: 4.5px solid var(--axdg-border-color-base);

    ${({ sort }) => {
      if (sort === 'asc') {
        return css`
          border-bottom: 4.5px solid var(--axdg-primary-color);
        `;
      }
    }}
  }
